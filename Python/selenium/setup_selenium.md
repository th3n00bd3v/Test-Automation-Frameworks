...
A basic enterprise-level test-data driven test suite for the web app 'https://eaapp.somee.com created by  @executeautomation.
...
5. Once done, test the virtual environment and setup by running the `test.py` file via the command `python -m test`.
6. Enter `deactivate` to exit the virtual environment.

 Note: Make sure to update the `requirements.txt` file often for resolving any dependancy conflicts.
